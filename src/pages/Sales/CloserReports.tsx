...
import { 
  ...
  BarChart3, DollarSign, Award, Sparkles, RefreshCw, Download,
  ChevronLeft, ChevronRight, Eye, Edit, Trash2
} from 'lucide-react';
import { CloserReport, CloserReportFormData, DailyCloserSummary } from '../../types/closerReports';
import { useCloserReports } from '../../hooks/useCloserReports';
import { useAuth } from '../../contexts/AuthContext';
import CloserReportForm from '../../components/CloserReports/CloserReportForm';
import RoleGuard from '../../components/RoleGuard';

const CloserReports: React.FC = () => {
  const { user } = useAuth();
  const { reports, loading, error, fetchReports, createReport, fetchDailySummary } = useCloserReports();
  const [showForm, setShowForm] = useState(false);
  const [dailySummary, setDailySummary] = useState<DailyCloserSummary[]>([]);
  ...
  const handleSaveReport = async (reportData: CloserReportFormData) => {
    try {
  ...
  const todayMetrics = todayReports.reduce((acc, report) => ({
    ...
    totalCalls: acc.totalCalls + report.calls_completed,
    totalRevenue: acc.totalRevenue + report.revenue_generated,
    totalCashCollected: acc.totalCashCollected + report.cash_collected,
    totalDeposits: acc.totalDeposits + report.deposits,
    totalMeetings: acc.totalMeetings + report.sales_team_meetings + report.leadership_meetings + report.training_sessions
  }), {
    totalReports: 0,
    totalCalls: 0,
    totalRevenue: 0,
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-400 mt-1">
                  Track daily sales performance and closing activities.
                </p>
              </div>
            </div>
          </div>
              ...
              <button
                ...
                className={`
                  ...
                  ${selectedDate === period.key
                    ? 'bg-gradient-to-r from-purple-400 to-blue-600 text-white shadow-lg shadow-purple-500/25'
          ...
          <div className="p-6 border-b border-gray-700">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold gradient-text section-gradient-sales">
                Closer Reports - {selectedDate.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
              </h2>
              <div className="flex items-center space-x-4">
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      <div className="space-y-1">
                        ...
                        <div className="text-xs text-green-400">
                          {report.total_calls_scheduled > 0 
                            ? `${Math.round((report.calls_completed / report.total_calls_scheduled) * 100)}%`
                            : '0%'
                          } completion
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-5">
                      <div className="space-y-1">
                        ...
                        <div className="text-xs text-gray-400">
                          {report.revenue_generated > 0 
                            ? `${Math.round((report.cash_collected / report.revenue_generated) * 100)}%`
                            : '0%'
                          } collected
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-5">
                      <span className="text-cyan-400 font-semibold">
                        {report.sales_team_meetings + report.leadership_meetings + report.training_sessions}
                      </span>
                    </td>
                    <td className="px-6 py-5">
